function removeShadow(I, mask, smoothEdges)
 % connected components found in the binary image, connectivity of 8
comp = bwconncomp(mask,8);
%NumObjects Find the maximum value of the label matrix
for i=1:comp.NumObjects
    
    % 0 indicates data to be ignored and 1 indicates data to keep
    maskTmp = zeros(size(mask));
   % calculate the mean value of pixels
    for j=1:size(comp.PixelIdxList{i},1)
        maskTmp(comp.PixelIdxList{i}(j)) = 1;
    end
    %Calculate boundaries of regions in maskTmp
    ...
    for j=1:n
        ...
        %Find the four-quadrant inverse tangent of the point
        t(j) = atan2(sy, sx);
    end
...
% remove the edge between the shadow and the light 
